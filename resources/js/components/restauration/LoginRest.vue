<template>
    <div class="container form_prof" id="content">
        <form>
            <h2 class="mb-3">Connexion Restauration </h2>
            <div class="input">
            ...
            <div class="row marg">
                ...
                <div class="col-sm-3 alternative-option">
                    <router-link class="nav-link" to="/registerRest">Inscription</router-link>
                </div>
            </div>
        </form>
    </div>
...
const handleLogin = async () => {
    if (
        ...
        window.Swal.fire({
            icon: 'error',
            title: 'Erreur',
            text: 'Veuillez remplir tous les champs pour connecter.',
        });
        return;
    }

    const getrest=async(idp)=>{
        const rep = await api.get(`/api/Rest/${idp}`)
