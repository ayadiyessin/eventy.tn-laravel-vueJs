<template>
    <div class="container form_prof" id="content">
        <form>
            <h2 class="mb-3">S'inscrire Restauration</h2>

            <div class="row">
                <div class="col-md-6">
                    <div class="input">
                        <label for="nom_rest">Nom de Restauration</label>
                        <input class="form-control" type="text" v-model="restauration.nom_rest" name="nom_rest" required />
                    </div>
                </div>
                <div class="col-md-6">
            ...
            <div class="row marg">
                ...
                <div class="col-sm-3 alternative-option">
                    <router-link class="nav-link" to="/loginRest">Connexion</router-link>
                </div>
            </div>
        </form>
    </div>
...
const addUser = async () => {
    try {
        ...
        restauration.value.usersID = us.value.id;
        telephone.value.usersID = us.value.id;

        await api.post('/api/Restauration/', restauration.value);

        await api.post('/api/telephones/', telephone.value);

        //router.replace({ name: 'LoginOrg' })
        router.push("/LoginRest")
