<template>
    <div class="container form_prof" id="content">
        <form>
            <h2 class="mb-3">Connexion Organisateur</h2>
            <div class="input">
            ...
            <div class="row marg">
                ...
                <div class="col-sm-3 alternative-option">
                    <router-link class="nav-link" to="/registerOrg">Inscription</router-link>
                </div>
            </div>
        </form>
    </div>
...
const handleLogin = async () => {
    ...
    const getorg=async(idp)=>{
        const rep = await api.get(`/api/Org/${idp}`)
